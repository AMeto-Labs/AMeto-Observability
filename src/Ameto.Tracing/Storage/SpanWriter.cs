...
internal static class SpanWriter
{
        ...
        List<byte[]>                             blooms)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // validation — SpanBloom.TryAddBlob returns false unless the blob is exactly one
            ...
            //
            // THE BLOOM IS FED FROM THE BYTES (TS#12): keys hashed as stored, string values folded
            // straight off their UTF-8, numbers formatted into the stack — no key string, no box,
            // no number text per attribute. That became possible once the hash stopped being
            // `value.ToString()` in the process culture (#86); see SpanBloom.
            var attrBlob = s.AttributesBytes;
            if (!attrBlob.IsEmpty && SpanBloom.TryAddBlob(bloomHashes, attrBlob))
            {
