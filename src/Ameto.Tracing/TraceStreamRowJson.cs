...
/// the same row (issue #93). One per stream.
///
...
/// generated <see cref="TraceStreamJson"/> contract through a writer over
/// <see cref="TraceDetailJson.WriterOptions(HttpContext)"/> — the host's encoder and layout, the
/// options the REST answers are written with — and handed to <see cref="SseJsonWriter"/> as a
...
/// System.Text.Json ships escapes the C0 controls, so no raw CR or LF can end the line early, and
...
internal sealed class TraceStreamRowJson : IDisposable
{
    ...
    public TraceStreamRowJson(HttpContext ctx) =>
        _json = new Utf8JsonWriter(_row, TraceDetailJson.WriterOptions(ctx));
