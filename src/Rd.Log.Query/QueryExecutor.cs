...
public sealed class QueryExecutor : IQueryExecutor
{
    ...
    private async IAsyncEnumerable<LogEvent> MergeColdSegmentsAsync(
        ...
        HashSet<Rd.Log.Core.LogLevel>?       levels,
        DateTimeOffset?                      from,
        DateTimeOffset?                      to,
        long?                                afterTs,
        Rd.Log.Core.EventId?                 afterId,
        bool                                 forward,
    ...
    {
        var trigramHints = filter.GetTrigramHints().ToList();
    ...
    /// </summary>
    private static bool AfterCursor(LogEvent ev, long? afterTs, Rd.Log.Core.EventId? afterId, bool forward)
    {
