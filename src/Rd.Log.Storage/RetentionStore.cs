...
using Rd.Log.Core;
using CoreLogLevel = Rd.Log.Core.LogLevel;

namespace Rd.Log.Storage;

/// <summary>
/// Persists the active <see cref="RetentionPolicy"/> in the shared
/// <c>rdlog.db</c> SQLite database (table <c>retention</c>).
...
public sealed class RetentionStore
{
    ...
    {
        _logger  = logger;
        _connStr = $"Data Source={Path.Combine(options.DataDirectory, "rdlog.db")}";
        InitTable();
