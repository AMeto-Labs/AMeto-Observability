...
// ── Configuration ─────────────────────────────────────────────────────────────
var rdLogSection = builder.Configuration.GetSection("RdLog");

// Auto-bind the entire RdLog section to ServerOptions; class defaults are the fallback.
var serverOptions = rdLogSection.Get<ServerOptions>() ?? new ServerOptions();
...
builder.Services.AddRdLogAuth(serverOptions.DataDirectory);
...
{
    var logger    = app.Services.GetRequiredService<ILoggerFactory>().CreateLogger("Rd.Log");
