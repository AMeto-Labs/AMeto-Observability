...
public static class OtlpTraceMapper
{
    ...
    private static SpanIngestItem? MapSpan(OtlpSpan span, string serviceName)
    {
        ...
        // Drop outbound HTTP spans that are calls to Ameto's own ingestion endpoints.
        // These appear when the app has AddHttpClientInstrumentation() and the Ameto
        // Serilog sink (or any OTLP exporter) ships telemetry back to this server.
        if (span.Kind == 3 /* CLIENT */ && IsAmetoInternalSpan(span.Attributes))
            return null;
    ...
    /// <summary>
    /// Returns <c>true</c> for outbound CLIENT spans whose URL targets Ameto's own
    /// ingestion or OTLP endpoints (/api/events, /otlp/v1/*).
    /// </summary>
    private static bool IsAmetoInternalSpan(List<OtlpKeyValue>? attrs)
    {
