...
<#
.SYNOPSIS
    Pack and push the Serilog.Sinks.Ameto NuGet package.

.DESCRIPTION
    1. dotnet pack -c Release  -> ./artifacts/nuget/Serilog.Sinks.Ameto.<Version>.nupkg
...
$root      = $PSScriptRoot
$proj      = Join-Path $root "src\Serilog.Sinks.Ameto\Serilog.Sinks.Ameto.csproj"
...
Write-Host "  Serilog.Sinks.Ameto  ·  NuGet publish" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
$nupkg = Get-ChildItem $outFull -Filter "Serilog.Sinks.Ameto.*.nupkg" `
...
if ($CopyTo) {
    ...
    $snupkg = Get-ChildItem $outFull -Filter "Serilog.Sinks.Ameto.*.snupkg" `
