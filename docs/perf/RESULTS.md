...
- ~~The segment index cache is the biggest remaining query win~~ — done in #80 (branch `perf/80-index-cache-hits`). An index group is now read lazily from its packed sections: a property catalog, one bucket or a handful of trigrams decoded per question. The cache keeps a memo per group of what queries learned, kilobytes each, instead of the decoded group (tens of MB). On the #80 stand (`IndexCacheHitProbe`: 1.2 M loggen events, 24 segments), a repeated `@mt like '%timeout%'` goes from 2.1 % hits, 925 ms and 834 MB per query to 100 % hits, 27–32 ms and 33 MB, with the whole cache at 1.35 MB. The uncached query goes from 968 ms to 56–65 ms. The 512 MB stand's derived 46 MB now hits 100 %, where it held nothing before, so it stays on. `MemoryBudgets`' comments that quote bloom as "4.1 % / 8.3 % of a cache entry" describe the decoded reader and are historical; the native ceiling no longer binds on the query path. What is left on a cache miss is reading the sections out of the mapped file (`SegmentReader` section rents at ~1.3 GB/s), which a zero-copy view over the mapping would remove.
