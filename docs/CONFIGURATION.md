...
**Upgrading needs nothing.** Segments written by an older release keep their old blooms and stay fully readable, and nothing rewrites them for this reason. This release reads an old bloom with the old hash, and where the old bloom may have recorded the value differently it checks only that the attribute key is present — for a string comparison whose literal looks like a number with a sign, decimal point, exponent, `NaN` or `Infinity` (`{ .ratio = "0.375" }`, `{ .skew = "-3" }`), and for one containing the few Greek and archaic Cyrillic letters (`σ`/`ς`, `µ`/`μ`, U+1C80–U+1C88…) whose case the old bloom folded differently from TraceQL. Those queries read more blocks of old segments; no query loses a row. Old segments leave as retention expires them (traces retention, `TracesDays`: 14 days by default — see Retention above; Settings → Retention after first run). Compaction rewrites only small segments, so count on the whole retention window, not on compaction.
