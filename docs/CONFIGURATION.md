...
**The cache has a second, native ceiling — not settable.** An entry is not all one kind of memory: its decoded postings are managed, but the bloom filter's bits are native (15–27 % of an entry), so they sit outside the GC heap limit that `IndexCacheBytes` is a share of when unset. That native share is bounded separately at `min(96 MB, 5 % of the physical limit)` — 25.6 MB in a 512 MB container — and whichever ceiling is reached first evicts from the least-recently-used end. Raising `IndexCacheBytes` deliberately does **not** raise it: more budget buys more decoded postings, which the GC can see and reclaim. Both figures are reported by `GET /api/diagnostics` as `indexCacheNativeBytes` and `indexCacheNativeBudgetBytes`. Under RAM pressure (see `RamTargetPercent`) the whole cache is now dropped along with the hot-tier flush — queries re-read what they need — and `indexCacheShedEvicted` counts how many entries that has cost.
