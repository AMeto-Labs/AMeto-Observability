...
### Metric write-ahead log (`metrics/metrics.wal`), v1 → v2

The metric WAL gained a CRC32C per entry, and per series record in `metrics.wal.pool` (format v2). After an unclean stop, replay now ends at the first entry that does not verify instead of replaying a torn one as a real point — the way a garbage point stamped in the year 2116 came back on every restart and was flushed into permanent `.mts` files. An entry grows by 4 bytes (48 → 52 for a scalar point), the file header from 32 to 64: it now records a commit's move of the surviving points, so a process killed in the middle of that move is finished at the next start instead of losing them, and it carries its own checksum — a header that does not verify has its counters rebuilt from the entries, with an Error, instead of hiding them. The log is still not fsynced between flushes: a power loss can still cost the points since the last flush; what it can no longer do is turn what a v2 log wrote into garbage.

The first start of a release that writes v2 **upgrades** an existing v1 log in place (a copy to `metrics.wal.upgrade.tmp`, then an atomic rename) — every point it holds is kept and replayed. The upgrade cannot repair what a crash under the old release already tore: a v1 entry has no checksum, so the upgrade computes one over whatever the entry holds. It drops the entries v1's own checks refuse — a generation or series index no append could have written, and a timestamp more than a day in the future (a Warning counts those) — but a torn v1 entry whose fields all look plausible is indistinguishable from a real point and is carried over, as the old release would have replayed it. The pool is not rewritten: its records say which version they are, so the old ones stay readable beside the new. The rename is made durable before the log is used — write-through on Windows, an fsync of the directory on Linux (macOS gets a plain fsync); if that sync fails, a Warning says so, and until the file system commits the rename on its own a power loss can bring the v1 log back under the name. If the upgrade cannot complete (a full disk, or an antivirus scanner holding the copy open), the server still starts: the log stays v1 for that run, an Error beginning `The metric WAL at` is logged, and the upgrade is retried at the next start.

**Rolling back is not symmetric.** A release older than v2 does not know the v2 layout: it treats `metrics.wal` as a foreign file and **re-initialises it** — and, the log being empty then, empties `metrics.wal.pool` too. Every point the log held and no `.mts` file did is **lost** by the rollback:

- after an *unclean* stop (crash, kill, power loss): every point since the last flush;
- after a *clean* stop: none, provided the final flush ran and nothing arrived behind it. The stop runs a final flush that writes the hot tier to `.mts` and empties the log of what it wrote, but metric ingest stays open while it runs, so points that arrive during that flush are logged and left for the next start; and a stop whose log has the Error `Final metric flush failed during shutdown` or `Failed to flush metric hot tier` left the whole tier in the log.

To roll back safely, stop the exporters sending to this node (or take it out of their path), then stop the newer release and check its log for those two Errors. Start it once more: a start that logs `Recovered N metric point(s) from the write-ahead log` found the previous stop's log not empty — stop it again (still without traffic) and repeat until a start logs no such line, then downgrade. A log that stayed v1 because its upgrade failed (the Error above) is read by the older release as it is.
