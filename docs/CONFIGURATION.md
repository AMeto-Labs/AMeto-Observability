...
### Span segment attribute blooms (`traces/*.trc`), hash v1 → v2

Every span segment carries a small bloom filter per block of 4 096 spans, which lets a TraceQL attribute query (`{ .db.system = "mssql" }`) skip blocks that cannot match. From this release the bloom records each value by its **culture-independent text** (`0.375`, `-3`, `1E+21`, `True`) folded the way TraceQL compares strings, instead of the text the server's locale produced — before, a segment flushed under a comma-decimal locale (ru-KZ, sv-SE…) and queried under another one could skip the block that held `{ .sampling.ratio = "0.375" }`. The segment format version (v3/v4) does not change: the new blooms follow a marker at the end of the segment's bloom section, behind one empty legacy slot per block.

**Upgrading needs nothing, and costs query time on old segments.** Segments written by an older release keep their old blooms and stay fully readable, and nothing rewrites them for this reason. The old bloom recorded each value lowercased by the casing tables of whichever host wrote it, with numbers in that host's locale, and neither was recorded, so this release trusts an old bloom's *value* only for a literal that is plain ASCII and not number-shaped (`{ .db.system = "mssql" }`). For every other string comparison it checks only that the attribute key is present, which means it reads every block of an old segment that carries the key:

* a literal with **any non-ASCII character**. That includes **every Cyrillic or Kazakh literal** (`{ .city = "Алматы" }`), and Greek, accented Latin, CJK and emoji ones. On a deployment whose attribute values are mostly Cyrillic, that is most value queries.
* a literal that looks like a number with a sign, decimal point, exponent, `NaN` or `Infinity` (`{ .ratio = "0.375" }`, `{ .skew = "-3" }`).

No query loses a row. The cost is read time: measured at about 3.6 ms per 4 096-span block (Release, warm cache), a full 50 000-span segment is about 13 blocks, roughly 45 ms per segment a query reads in full instead of one or two blocks. This lasts until retention expires the old segments (traces retention, `TracesDays`: 14 days by default; see Retention above, or Settings → Retention after first run). Compaction rewrites only small segments, so plan on the whole retention window, not on compaction. Segments written by this release skip blocks for these literals as well. The one exception is a host whose case tables are newer than the ones compiled into the build (a later ICU or .NET): there, a literal containing a character whose casing that host changed checks only the key. The build detects this on its own. It was measured not to happen on Windows with ICU or under the Docker image's invariant globalization; a host with an older ICU than the build's tables is fine as well.

**Rolling back is safe for the data.** An older release finds an empty bloom in each legacy slot, which every release treats as "no bloom — read the block", so on segments written by this release its TraceQL attribute queries read every block — slower, and the same rows. Nothing is rewritten, flagged or deleted by the rollback, and upgrading again restores the skip.

One visible change comes with it: a quoted string compared with a numeric attribute now always compares the number's invariant text, whatever locale the server runs under — `{ .ratio = "0.375" }` matches, and `{ .ratio = "0,375" }` no longer does on a comma-decimal host.
