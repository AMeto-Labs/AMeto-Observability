...
```json
{
  ...
  "metricsLabelPoolStrings": 596,
  "metricsLabelPoolMaxStrings": 16384,
  "metricsLabelPoolSaturations": 0,
  "metricsLabelPoolResets": 0,
...
**Values JSON cannot hold.** A point's `value` or `sum` that is NaN or ±Infinity — legal in OTLP (an exporter dividing by zero, an empty histogram's mean) — is answered as `null`, per point; the rest of the series and of the answer is unaffected. The same holds for an exemplar's `value` and a heatmap's `bounds` / `counts`. (These used to fail the whole request with a 500, or drop the connection part-way through a large `GET /api/metrics/{name}`.) Aggregations (`sum`, `avg`, `min`, `max`, expressions) reduce over the finite values only; a timestamp where no series has one is `null`. `rate` / `increase` keep one point per sample after the first: a non-finite sample's point is `null`, and the next rate runs from the last finite sample. `last` reads each series' latest sample only — a series whose latest sample is non-finite contributes nothing (never an older value), and a group with no finite latest sample is `null`; top-K ranks such a series last. A metric alert rule skips such points, and a window holding nothing else leaves the rule's state unchanged; the server logs it once per rule. `POST /api/alerts/preview` then answers `"value": null, "wouldFire": false`.

**Label keys are unique within a series.** OTLP forbids an attribute key twice on one data point, but exporters send it — the same attribute twice, or `service.name` on the point as well as the resource. At ingest the last value of a repeated point attribute wins, and a point attribute wins over the resource's value for the same key, so a series never carries a key twice. A series stored before this rule is answered with the key once, with its **ordinal-greatest** value — the order the values were sent in was never stored, so this is not necessarily the value ingest keeps today — and `filters`, `groupBy` and alert-rule `labels` match and group it on that same value. For example, a series stored with the point attribute `service.name=api` under a resource `service.name=gateway` is answered and filtered as `service.name=gateway`, while the same series ingested now is `service.name=api` (the point wins): the two are different series.
