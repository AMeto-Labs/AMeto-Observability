...
namespace Rd.Log.Perf;

// ── BenchmarkDotNet runner ────────────────────────────────────────────────────

/// <summary>
/// Entry point: run "dotnet run -c Release" from the Rd.Log.Perf project directory.
...
{
    private Rd.Log.Query.Filtering.CompiledFilter _filter = null!;
    ...
    public void Setup()
    {
        _filter = Rd.Log.Query.Filtering.CompiledFilter.Compile(
...
public sealed class PerfSmokeTests : IDisposable
{
    ...
    {
        var filter = Rd.Log.Query.Filtering.CompiledFilter.Compile("@l = 'Error'");
        int errors = 0;
