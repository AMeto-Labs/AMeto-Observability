...
public sealed class NodeDispatchTests
{
    ...
    public void The_bulk_shapes_carry_their_tag()
    {
        ...
        Assert.Equal(NodeKind.TimeCompare, new TimeCompareNode(CompareOp.Ge, 1L).DispatchKind);
        Assert.Equal(NodeKind.TraceIdCompare,
                     new TraceIdCompareNode(CompareOp.Eq, isSpan: false, hi: 1UL, lo: 2UL, property: "@tr").DispatchKind);
    }
