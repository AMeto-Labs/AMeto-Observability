...
public sealed class IngestionQueryIntegrationTests
    : IClassFixture<RdLogWebAppFactory>
{
    private readonly RdLogWebAppFactory _factory;
    private readonly HttpClient         _client;

    public IngestionQueryIntegrationTests(RdLogWebAppFactory factory)
    {
        _factory = factory;
...
/// </summary>
public sealed class RdLogWebAppFactory : WebApplicationFactory<Program>
{
    private readonly string _tempDir = Path.Combine(Path.GetTempPath(), "rdlog-tests-" + Guid.NewGuid().ToString("N")[..8]);

    protected override void ConfigureWebHost(Microsoft.AspNetCore.Hosting.IWebHostBuilder builder)
    {
        builder.UseSetting("RdLog:DataDirectory", _tempDir);
        builder.UseSetting("RdLog:ApiKey", "");          // no auth for tests
        builder.UseSetting("RdLog:HttpPort", "0");       // random port
        builder.UseSetting("RdLog:Cluster:Enabled", "false");
