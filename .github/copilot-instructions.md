# Ameto — Copilot Instructions

Ameto is a **high-performance, self-hosted structured log server** — a self-contained alternative to Datalust Seq.
...
```
src/
  Ameto.Core/          ← Value types, core interfaces (IQueryExecutor, ISegmentProvider…)
  Ameto.Storage/       ← Hot/cold tier, WAL, LZ4 segments, NativeMemory arenas
  Ameto.Ingestion/     ← MPMC lock-free ring buffer, IngestionEndpoint, backpressure
  Ameto.Indexing/      ← Inverted index, bloom filter, trigram index (built post-flush)
  Ameto.Query/         ← Filter expression evaluation, SSE streaming, segment merging
  Ameto.Alerts/        ← Alert rules (SQLite), threshold eval, webhook/SMTP dispatch
  Ameto.Replication/   ← Symmetric segment replication, peer discovery
  Ameto.Otel/          ← OTLP/HTTP endpoints, JSON+Protobuf decoders, OtlpEndpointMapper
  Ameto.Tracing/       ← Separate trace storage, TraceQueryEndpointMapper
  Ameto.Metrics/       ← Separate metric storage (hot/cold .mts files), MetricQueryEndpointMapper
  Ameto.Serilog/       ← Serilog sink for self-ingestion
  Ameto.Server/        ← ASP.NET Core host, Program.cs, all *EndpointMapper registrations
tests/
  Ameto.Core.Tests/
  Ameto.Storage.Tests/
  Ameto.Query.Tests/
  Ameto.Integration.Tests/
  Ameto.Perf/          ← BenchmarkDotNet + CI smoke tests
...
```csharp
// 1. DI registration  (src/Ameto.Foo/FooServiceExtensions.cs)
public static class FooServiceExtensions {
    public static IServiceCollection AddAmetoFoo(this IServiceCollection services) { ... }
}

// 2. Endpoint mapping  (src/Ameto.Foo/FooEndpointMapper.cs  OR  src/Ameto.Server/...)
...
```csharp
builder.Services
    .AddAmetoStorage()    // must be first
    .AddAmetoIndexing()   // after storage
    .AddAmetoIngestion()  // after storage
    .AddAmetoQuery();     // after storage + indexing
...
### Core interfaces (`src/Ameto.Core/Interfaces.cs`)
...
### Zero-allocation rules (hot path only — `Ameto.Ingestion`, `Ameto.Storage`)
...
dotnet run -c Release --project tests/Ameto.Perf  # benchmarks
...
3. **New module** → create `FooServiceExtensions.AddAmetoFoo()` + call it in `Program.cs` in the correct order
...
8. **Configuration** uses YAML + env overrides (`Ameto__DataDirectory`); see [docs/CONFIGURATION.md](../docs/CONFIGURATION.md)
