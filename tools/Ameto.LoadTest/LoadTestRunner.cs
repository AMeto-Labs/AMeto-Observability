    ...
    ILogger<LoadTestRunner> logger)
{
    ...
    {
        using var http   = httpFactory.CreateClient("Ameto");
    ...
    private Ameto.Core.LogLevel PickLevel(Random rng)
    {
        int roll = rng.Next(0, 100);
        if (roll < _levelCdf[0]) return Ameto.Core.LogLevel.Verbose;
        if (roll < _levelCdf[1]) return Ameto.Core.LogLevel.Debug;
        if (roll < _levelCdf[2]) return Ameto.Core.LogLevel.Information;
        if (roll < _levelCdf[3]) return Ameto.Core.LogLevel.Warning;
        if (roll < _levelCdf[4]) return Ameto.Core.LogLevel.Error;
        return Ameto.Core.LogLevel.Fatal;
    }
